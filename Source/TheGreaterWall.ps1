...
function tgw ($rawcommand){
    ...
    function get-wsmanconnection ($listofips){
        if ($completedconnectiontest -ne "Yes"){
            ...
            End{}
                ...
                if ($unreachable){
                    clear-host
                    write-output "Warning. The following endpoints are unreachable. No modules will be attempted on these IPs."
    ...
    function go-interactive{
        ...
        write-output " "
        $endpoints= $listofips
        write-output "1.) Choose IP From list"
    ...
    function display-admincommands{
        ...
        write-output 'Command= "show-ips"               Description= Shows list of IP addresses that you previously defined'
        write-output 'Command= "reset-ips"              Description= Allows you to repeat the part where you input the credentials'
        write-output 'Command= "hail-mary"              Description= Runs all modules on all IPs, immediately'
    ...
    Function main-prompt{
        ...
        write-output "2.) Host Collection Obfuscated"
        Write-Output "3.) EventLog Collection"
        Write-Output "4.) EventLog Collection Obfuscated"
        Write-Output "5.) Post-process and analyze data"
        $choice= Read-Host -Prompt " "
    
        if ($choice -eq "1"){
            $modules= "$env:userprofile\desktop\thegreaterwall\modules\hostcollection"
            new-variable -name obfuscationmode -value "Disabled" -force -Scope global
        }
            
        if ($choice -eq "2"){   
            $modules= "$env:userprofile\desktop\thegreaterwall\modules\HostCollection_Obfuscated"
            new-variable -name obfuscationmode -value "Enabled" -force -Scope global
        }
        ...
        if ($choice -eq "4"){
            $modules= "$env:userprofile\desktop\thegreaterwall\modules\EventLogs_Obfuscated"
            new-variable -name obfuscationmode -value "Enabled" -force -Scope global
        }
        ...
        if ($choice -ne "1" -and $choice -ne "2" -and $choice -ne "3" -and $choice -ne "4" -and $choice -ne "5" -and $choice -ne "admin-commands"){
            clear-host
    ...
    function setup-framework{
        ...
        $env:PSModulePath= "$env:userprofile\documents\windowspowershell\modules;c:\windows\system32\windowspowershell\v1.0\modules;$env:userprofile\desktop\TheGreaterWall\modules;$env:userprofile\desktop\TheGreaterWall\modules\eventlogs;$env:userprofile\desktop\TheGreaterWall\modules\hostcollection;$env:userprofile\desktop\TheGreaterWall\modules\hostcollection_obfuscated;$env:userprofile\desktop\TheGreaterWall\modules\eventlogs_obfuscated"
        set-location $env:userprofile\desktop\thegreaterwall
    }
    ...
    function get-ipaddresses{
        ...
        if (!$listofips -or $action -eq "reset-ips"){
            ...
            if ($listofips -eq "1"){
                clear-host
                Header
                Write-Host "Please provide the file location of the IP addresses to investigate:`n"
                Write-Host "    Example: " -NoNewline `""$env:USERPROFILE\Desktop\ListofIPaddresses.txt"`"
                Write-Host "`n"
    ...
    function display-menu ($modulepath){
        ...
        try{
            ...
            if ($obfuscationmode -eq "Enabled"){
                $menu= $menu | % {$_-replace("_obfuscated","")}
            }
            ...
            if ($mode -like "*obfuscated*"){
                $m= $mode-replace('_obfuscated','')
            }

            if ($mode -notlike "*obfuscated*"){
                $m= $mode
            }
            ...
            write-output "[ Mode: $m | Obfuscation: $obfuscationmode | Credentials: $c | Targeted Endpoints: $($listofips.count) ]"
            write-output " "
    ...
    while ($true){
        ...
        if ($datatype -eq "String"){
            ...
            if ($action -eq "hail-mary"){
                ...
                write-output "Obfuscation: $obfuscationmode"
                Write-Host " "
                ...
                if ($choice -eq "1"){
                    ...
                    if ($obfuscationmode -eq "Disabled"){
                    ...
                    if ($obfuscationmode -eq "Enabled"){
                        $action= $(Get-childitem -Recurse $env:userprofile\Desktop\TheGreaterWall\Modules\hostcollection_obfuscated | where {$_.mode -like "*a*"}).name
                        $action= $action | % {$_.tostring()-replace('.psm1','')} | Sort -Unique
                        Set-Variable -name action -value $action -force -Scope global
                    }
                }
                ...
                if ($choice -eq "2"){
                    ...
                    if ($obfuscationmode -eq "Enabled"){
                        $action= $(Get-childitem -Recurse $env:userprofile\Desktop\TheGreaterWall\Modules\EventLogs_obfuscated | where {$_.mode -like "*a*"}).name
                        $action= $action | % {$_.tostring()-replace('.psm1','')} | Sort -Unique
                        Set-Variable -name action -value $action -force -Scope global
                    }
                }
            ...
            if ($action -eq "reset-obfuscationmode"){
                clear-host
                set-obfuscationmode
                Remove-Variable -name action -Force -ErrorAction SilentlyContinue
            ...
            if ($action -eq "show-obfuscationmode"){
                clear-host
                write-output "Obfsucation Mode is:"
                write-output "- $obfuscationmode"
                write-output " "
                pause
                Remove-Variable -name action -Force -ErrorAction SilentlyContinue
            }
        ...
        if ($action.count -gt 1){
            ...
            foreach ($a in $action){
                if ($a -like "*activedirectory*"){
                    ...
                    start-job -ScriptBlock $actioncode -name "$modulename-$date"
                }
                    ...
                    foreach ($ip in $listofips){
                        ...
                        invoke-command -ScriptBlock $actioncode -ComputerName $ip -Credential $credentials -JobName "$ip-$a-$date" -ArgumentList $ip -AsJob                                     
                    }                                            
                }            
                remove-variable -name action -Force -ErrorAction SilentlyContinue
            }                    
